...
class MessageSerializer(serializers.Serializer):
    ...
    def create(self, validated_data,):
        try:
            user_receiver = User.objects.filter(username=validated_data['receiver'])#nguoi nhan
            user_sender = User.objects.filter(username=self.context['request'].user)#nguoi nhan
            
            # user_sender= User.objects.filter(username=self.request.user)
            message = Message()
            
            # print(self.context['request'].user)
            message.text = validated_data['text']
            message.sender = user_sender.first()
            # message.sender = self.context['request'].user
            message.receiver = user_receiver.first()
